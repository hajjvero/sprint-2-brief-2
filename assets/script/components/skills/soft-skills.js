...
const inputSoftSkills = document.getElementById('soft-skills-input');
const buttonSoftSkillsSave = document.getElementById("soft-skills-save-button");
const buttonSoftSkillsClear = document.getElementById("soft-skills-clear-button");

export default function initSoftSkills() {
    // save button
    buttonSoftSkillsSave.addEventListener('click', () => {
        removeError(inputSoftSkills);
        if (isEmpty(inputSoftSkills.value.trim())) {
            displayError(inputSoftSkills, "Ce champ est requis.");
            return;
        }

        // edit
        if (inputSoftSkills.hasAttribute("data-skills-id")) {
            const index = +inputSoftSkills.getAttribute("data-skills-id");
            resumeObjet.skills.soft[index] = inputSoftSkills.value.trim();
            renderSoftSkills();
        }
        // add
        else {
            addSkills(inputSoftSkills.value.trim());
        }
    ...
    // clear button
    buttonSoftSkillsClear.addEventListener('click', () => {
        removeError(inputSoftSkills);
...
export const renderSoftSkills = () => {
    softSkillsContainer.innerHTML = "";
    softSkillsContainer.append(...resumeObjet.skills.soft.map(showSkills));
}

const addSkills = (skills) => {
    resumeObjet.skills.soft.push(skills);
...
const removeSkills = (index) => {
    if (confirm("Voulez-vous vraiment supprimer cette compétence ?")) {
        resumeObjet.skills.soft.splice(index, 1);
        renderSoftSkills();
    }
}

const updateSkills = (index) => {
    inputSoftSkills.setAttribute("data-skills-id", index);
    inputSoftSkills.value = resumeObjet.skills.soft[index];

    buttonSoftSkillsSave.firstElementChild.classList.add('hidden');
    buttonSoftSkillsSave.lastElementChild.classList.remove('hidden');
}
...
const clearInput = () => {
    inputSoftSkills.value = "";
    inputSoftSkills.removeAttribute("data-skills-id");
    buttonSoftSkillsSave.firstElementChild.classList.remove('hidden');
    buttonSoftSkillsSave.lastElementChild.classList.add('hidden');
    inputSoftSkills.focus();
}